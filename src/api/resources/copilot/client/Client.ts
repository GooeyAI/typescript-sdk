...
export class Copilot {
    ...
    /**
     * @param {File[] | fs.ReadStream[] | Blob[] | undefined} inputImages
     * @param {File[] | fs.ReadStream[] | Blob[] | undefined} inputDocuments
     * @param {File[] | fs.ReadStream[] | Blob[] | undefined} documents
     * @param {File | fs.ReadStream | Blob | undefined} inputGlossaryDocument
     * @param {File | fs.ReadStream | Blob | undefined} outputGlossaryDocument
     * @param {File | fs.ReadStream | Blob | undefined} inputFace
     * @param {Gooey.CopilotCompletionRequest} request
    ...
    public async completion(
    ...
    ): Promise<Gooey.VideoBotsPageOutput> {
        ...
        if (inputImages != null) {
            for (const _file of inputImages) {
                await _request.append("input_images", _file);
            }
        }

        if (inputDocuments != null) {
            for (const _file of inputDocuments) {
                await _request.append("input_documents", _file);
            }
        }
        ...
        if (inputGlossaryDocument != null) {
            await _request.appendFile("input_glossary_document", inputGlossaryDocument);
        }

        if (outputGlossaryDocument != null) {
            await _request.appendFile("output_glossary_document", outputGlossaryDocument);
        }
        ...
        if (_response.ok) {
            return serializers.VideoBotsPageOutput.parseOrThrow(_response.body, {
